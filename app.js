...
const app = {
  ...
  dict: {
    "Home": "Accueil",
    "Build your wardrobe": "Composez votre garde-robe",
    "Find your style": "Trouvez votre style",
    "Translate to French": "Translate to English",
    "Elevate Your Everyday ": "Élevez votre quotidien ",
    "Seamlessly.": "Sans effort.",
    "A curated capsule wardrobe blending precise French minimalism with vibrant Indian elegance.": "Une garde-robe capsule soigneusement sélectionnée alliant le minimalisme français à l'élégance indienne.",
    "Start Your Style Journey ": "Commencez votre voyage de style ",
    "Wardrobe": "Garde-robe",
    "Profile": "Profil",
    "Blend & Budget": "Rendu & Budget",
    "Select existing pieces you own": "Sélectionnez les pièces que vous possédez",
    "Indian Kurtas": "Kurtas Indiens",
    "Salwar Pants": "Pantalons Salwar",
    "Sarees": "Saris",
    "Dupattas": "Dupattas",
    "Classic Jeans": "Jeans Classiques",
    "Crisp White/Black Shirts": "Chemises Blanches/Noires",
    "Trench Coat": "Manteau Trench",
    "Tailored Blazer": "Blazer Ajusté",
    "Breton Stripe Top": "Haut à Rayures Bretonnes",
    "Wide-leg Trousers": "Pantalons Larges",
    "Other existing pieces you own": "Autres pièces que vous possédez",
    "Next ": "Suivant ",
    "Back": "Retour",
    "Body Type & Colors": "Type de Corps & Couleurs",
    "Body Type": "Type de Corps",
    "Color Palette": "Palette de Couleurs",
    "Pear Shape": "En Forme de Poire",
    "Rectangle": "Rectangle",
    "Hourglass": "Sablier",
    "Apple": "Pomme",
    ...
    "Petite": "Petite",
    "Tall": "Grande",
    "We've pre-selected beige, navy, and black based on typical capsules. Tap to select/deselect your favorites.": "Nous avons pré-sélectionné le beige, le bleu marine et le noir. Appuyez pour choisir vos favoris.",
    "The Perfect Blend": "Le Mélange Parfait",
    "Budget for new purchases (₹)": "Budget pour de nouveaux achats (₹)",
    "French vs Indian Style Blend": "Mélange de Style Français vs Indien",
    "French": "Français",
    "Indian": "Indien",
    " Minimalist French / ": " Français Minimaliste / ",
    " Vibrant Indian": " Indien Vibrant",
    "Generate Capsule ": "Générer la Capsule ",
    "Your Custom Capsule": "Votre Capsule Personnalisée",
    "A curated mix of your loved items & strategic new pieces.": "Un mélange de vos articles préférés et de nouvelles pièces stratégiques.",
    "Wardrobe Analysis": "Analyse de la Garde-robe",
    "Existing": "Existant",
    "Identified Gaps": "Lacunes Identifiées",
    "Smart Additions ": "Ajouts Intelligents ",
    "Generated Outfits": "Tenues Générées",
    "Which silhouette appeals to you the most?": "Quelle silhouette vous attire le plus ?",
    "Select your go-to weekend vibe:": "Sélectionnez votre ambiance de week-end :",
    "Pick a fabric you love to touch:": "Choisissez un tissu que vous aimez toucher :",
    "Structured & Tailored": "Structuré & Ajusté",
    "Flowy & Draped": "Fluide & Drapé",
    "Smart Casual": "Décontracté Élégant",
    "Comfortable Elegance": "Élégance Confortable",
    "A cafe in Paris": "Un café à Paris",
    "An artistic heritage walk": "Une promenade artistique",
    "Gallery hopping": "Visite de galeries",
    "Festive family gathering": "Fête de famille",
    "Crisp Linen": "Lin Frais",
    "Rich Silk/Chanderi": "Soie Riche/Chanderi",
    "Soft Cotton Rib": "Côtelé de Coton Doux",
    "Block Printed Cotton": "Coton Imprimé",
    "Style Matched! ": "Style Trouvé ! ",
    "Continue to Wardrobe Setup ": "Continuer vers la configuration ",
    "Analyzing Wardrobe... ": "Analyse de la Garde-robe... ",
    "Missing structured outerwear for layering": "Il manque des vêtements d'extérieur structurés",
    "Missing versatile, foundational bottoms": "Il manque des bas polyvalents de base",
    "Missing fluid draping elements for ethnic touch": "Il manque des éléments drapés pour une touche ethnique",
    "Missing classic minimalist foundational tops": "Il manque des hauts minimalistes classiques",
    "Wardrobe is solid! Need statement belts or scarves to elevate.": "Garde-robe solide ! Besoin d'accessoires pour rehausser.",
    "Perfectly closes your style gap": "Comble parfaitement votre lacune de style"
  },
  ...
  init() {
    ...
    for (let key in this.dict) {
      this.reverseDict[this.dict[key]] = key;
    }
    this.setupQuiz();
    this.setupListeners();
    this.showPage('home');
  ...
  // --- Translation Logic ---
  toggleLanguage() {
    this.state.lang = this.state.lang === 'en' ? 'fr' : 'en';
    const btn = document.getElementById('lang-btn');
    if (this.state.lang === 'fr') {
      btn.innerText = "Translate to English";
    } else {
      btn.innerText = "Translate to French";
    }
  ...
  translateDOM(node) {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      let sourceMap = this.state.lang === 'fr' ? this.dict : this.reverseDict;
      
      // Exact match
      if (sourceMap[text]) {
        node.nodeValue = sourceMap[text];
      } 
      // Trimmed match
      else if (sourceMap[text.trim()]) {
         node.nodeValue = text.replace(text.trim(), sourceMap[text.trim()]);
      }
    } else if (node.nodeType === Node.ELEMENT_NODE) {
      if (node.id === 'lang-btn') return; // Skip the translate button itself
      if (node.nodeName === 'SCRIPT' || node.nodeName === 'STYLE') return;
      
      // Translate Placeholders
      if (node.placeholder) {
        let sourceMap = this.state.lang === 'fr' ? this.dict : this.reverseDict;
        if (sourceMap[node.placeholder]) {
           node.placeholder = sourceMap[node.placeholder];
        }
      }
  ...
  t(str) {
    if (this.state.lang === 'en') return str;
  ...
  renderQuizStep() {
    ...
    document.getElementById('quiz-question').innerText = this.t(currentQ.question);
    ...
    currentQ.options.forEach((opt, idx) => {
      ...
      card.innerText = this.t(opt.text);
  ...
  populateDashboard() {
    ...
    document.getElementById('analysis-gaps').innerHTML = gaps.map(g => `<li>${this.t(g)}</li>`).join('');
    ...
    const budget = isNaN(parseInt(budgetRaw, 10)) ? 0 : parseInt(budgetRaw, 10);
    document.getElementById('budget-badge').innerText = `${this.t("Under")} ₹${budget.toLocaleString()}`;
    
    let additionsHTML = '';
    const colorKey = selectedColors[0].split(' ')[0]; // just grab the first word easily
    const color = this.t(colorKey) || colorKey;
    ...
    gaps.forEach((gap, index) => {
      ...
      else if (gap.includes('bottoms')) { itemType = 'Wide-leg Trousers'; icon = '👖'; }
      ...
      <div class="shop-item">
        ...
        <div class="shop-details">
          <h4>${color} ${this.t(itemType)}</h4>
          <p>${this.t("Perfectly closes your style gap")}</p>
        </div>
